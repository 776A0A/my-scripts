...
class Generator {
    ...
    extendPkg() {
        ...
        this.finalTemplate['package.json'] = JSON.stringify(merge(existingPkg, templatePkg), null, 4);
    }
    writeFileToLocal() {
        ...
        return new Promise((resolve, reject) => {
            ...
            childProcess.on('close', (code) => {
                if (code !== 0) {
                    ...
                    reject(Error(chalk.redBright('Error ocurred when installing deps!')));
                }
    ...
    resolveConflictDepMainVersion(existingPkg, templatePkg) {
        ...
        depKeys.forEach((key) => {
            const existingDeps = existingPkg[key], templateDeps = templatePkg[key];
            if (!existingDeps || !templateDeps)
                return;
    ...
    compareDeps(existingDeps, templateDeps) {
        ...
        existingDepsKeys.forEach((depKey) => {
            ...
            this.exitIfError(existingDepMainVersion !== templateDepMainVersion, `The two main version of ${chalk.redBright(depKey)} are conflict!`);
        });
    ...
    isPkgExisting() {
        ...
        this.exitIfError(!isExisting, `没有检测到 package.json，请使用 ${chalk.redBright('npm init')} 进行初始化！`);
    }
}
